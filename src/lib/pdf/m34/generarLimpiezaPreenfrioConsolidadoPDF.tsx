...
import {
  LimpiezaPreenfrioConsolidadoPDF,
...
async function construirPagina(id: string, orgId: string, codigoClave: string): Promise<LimpiezaPreenfrioPaginaProps> {
...
export async function generarLimpiezaPreenfrioConsolidadoPDF(
  ranchoId: string,
  ranchoNombre: string,
  orgId: string,
  desde: string,
  ...
  const { data, error } = await tbl('m34_registro_mensual')
    .select('id, anio, mes')
    .eq('org_id', orgId)
  ...
  a.href = url
  a.download = `Limpieza_Preenfrio_Conservador-consolidado-${desde}-${hasta}.pdf`
  a.click()
