...
const D_COL_WIDTHS = { dia: 20, conc_cloro: 60, aj_cloro: 60, conc_acido: 65, aj_acido: 60, realizo: 100, aprobo: 100 }

function valorDisplay(v: ValorM33PDF | undefined): { text: string; color: string } {
...
export function LimpiezaRecepcionPagina({ instalacion, instalacionCodigo, anio, mes, items, resultados, diasData, observaciones, codigoClave }: LimpiezaRecepcionPaginaProps) {
    ...
    <Page size="A4" orientation="landscape" style={s.page}>
      <View style={s.headerRow}>
        ...
        <View style={s.titleBlock}>
          <Text style={s.title}>BITACORA DE LIMPIEZA DEL AREA DE RECEPCION</Text>
          <Text style={s.subtitle}>{mesLabel}</Text>
        </View>
        <Text style={s.codigoText}>Codigo: {codigoFormato('F-FRUS-SAN-11', codigoClave)}</Text>
      </View>
      ...
      <View style={s.infoRow}>
        ...
        <View style={s.infoCell}>
          ...
          <Text style={s.infoValue}>Area de recepcion</Text>
        </View>
      ...
      {diasDataEntries.length > 0 && (
        <>
          <Text style={s.sectionTitle}>Registro diario de concentraciones y personal</Text>
          <View style={s.dTable}>
            <View style={s.dHdrRow}>
              <View style={[s.dCell, { width: D_COL_WIDTHS.dia }]}><Text style={s.dHdrText}>Dia</Text></View>
              <View style={[s.dCell, { width: D_COL_WIDTHS.conc_cloro }]}><Text style={s.dHdrText}>Conc. Cloro (ppm)</Text></View>
              <View style={[s.dCell, { width: D_COL_WIDTHS.aj_cloro }]}><Text style={s.dHdrText}>Ajuste Cloro</Text></View>
              <View style={[s.dCell, { width: D_COL_WIDTHS.conc_acido }]}><Text style={s.dHdrText}>Conc. Ac. Per. (ppm)</Text></View>
              <View style={[s.dCell, { width: D_COL_WIDTHS.aj_acido }]}><Text style={s.dHdrText}>Ajuste Ac. Per.</Text></View>
            ...
            {diasDataEntries.map((dia, idx) => {
              ...
              return (
                <View key={dia} style={[s.dRow, { backgroundColor: idx % 2 === 1 ? ROW_ALT : WHITE }]}>
                  ...
                  <View style={[s.dCell, { width: D_COL_WIDTHS.conc_cloro }]}><Text style={s.dText}>{d.concentracion_cloro != null ? String(d.concentracion_cloro) : ''}</Text></View>
                  <View style={[s.dCell, { width: D_COL_WIDTHS.aj_cloro }]}><Text style={s.dText}>{d.ajuste_cloro ?? ''}</Text></View>
                  <View style={[s.dCell, { width: D_COL_WIDTHS.conc_acido }]}><Text style={s.dText}>{d.concentracion_acido != null ? String(d.concentracion_acido) : ''}</Text></View>
                  <View style={[s.dCell, { width: D_COL_WIDTHS.aj_acido }]}><Text style={s.dText}>{d.ajuste_acido ?? ''}</Text></View>
      ...
      <View style={s.footer}>
        <Text style={s.footerText}>{codigoFormato('F-FRUS-SAN-11', codigoClave)}</Text>
...
export function LimpiezaRecepcionConsolidadoPDF({ paginas }: LimpiezaRecepcionConsolidadoProps) {
  return (
    <Document>
      {paginas.map((p, i) => (
        <LimpiezaRecepcionPagina key={i} {...p} />
      ))}
    </Document>
  )
}
