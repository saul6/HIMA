...
export interface MipIntervencionItemRow {
  numero: number | null
  texto: string
...
export interface MipIntervencionPDFProps {
  rancho: string
...
export function MipIntervencionPDFPage({
  rancho,
  orgNombre,
  fecha,
  producto,
  ...
  return (
    ...
    >
      ...
      <PdfSectionBanner>2. Matriz de tecnicas de intervencion</PdfSectionBanner>
      ...
      <View style={{ marginTop: 4, borderWidth: 1, borderColor: PC.border }}>
        <View style={{ flexDirection: 'row', backgroundColor: PC.section }}>
          ...
          <View style={[thStyle, { flex: 1, backgroundColor: PC.section, alignItems: 'flex-start' }]}>
            <Text style={{ fontFamily: 'Helvetica-Bold', fontSize: 6, color: PC.white }}>Tecnica de intervencion</Text>
          </View>
...
export function MipIntervencionPDF(props: MipIntervencionPDFProps) {
  return (
    <Document>
      <MipIntervencionPDFPage {...props} />
    </Document>
  )
}
